...
export async function replyToTripGuide(
  ...
  const nextStop =
    question && /下一站/.test(question) ? trip?.guideDestination : undefined;
      ...
      : (
          await mobileChat(
            [
              ...
              {
                role: "user",
                content: question || `请讲述${guide.stopName}的故事`,
              },
            ],
            { model: session.model, maxTokens: 1600 },
          )
        ).content;
...
async function tellArrivalStory(session: LocalChatSession) {
  ...
  const update = (content: string, status: GuideContext["status"]) =>
  ...
  try {
    const text = await replyToTripGuide(session, undefined, (content) =>
      update(content, "loading"),
    );
    update(text, "ready");
  } catch {
    update(
      "景点讲解暂时未能完成，请检查 QwenPaw 连接后回复“重试讲解”。已有聊天和旅程已保留。",
      "error",
    );
  } finally {
...
export function startTripGuideRuntime(
  ...
  try {
    const raw: unknown = JSON.parse(
      localStorage.getItem(GUIDE_VISITS_KEY) || "[]",
    );
    if (Array.isArray(raw))
      raw
        .filter((item) => typeof item === "string")
        .forEach((key) => visits.add(key));
  } catch {
    /* invalid visit state cannot create an unsafe GPS match */
  }
  ...
  updateChatSessions((sessions) =>
    sessions.map((session) =>
      session.guide?.status === "loading" && !runningStories.has(session.id)
        ? {
            ...session,
            guide: { ...session.guide, status: "error" },
            messages: [
              ...session.messages,
              {
                id: createId("message"),
                role: "assistant" as const,
                content: "上次讲解被中断，可以回复“重试讲解”。",
                createdAt: Date.now(),
              },
            ],
          }
  ...
  const onError = (text: string) =>
    window.dispatchEvent(new CustomEvent(GUIDE_ERROR_EVENT, { detail: text }));
  const onPosition = (tripId: string, position: TripPosition) => {
    if (stopped) return;
    const trips = loadTrips();
    const trip = trips.find(
      (item) => item.id === tripId && item.status === "active",
    );
    if (!trip) return;
    ...
    const arrival = choosingDestination
      ? null
      : tracker.observe(trip, position, visits);
    const updated = {
      ...trip,
      lastPosition: position,
      currentStopIndex:
        arrival && arrival.index >= 0 ? arrival.index : trip.currentStopIndex,
    };
    saveTrips(trips.map((item) => (item.id === tripId ? updated : item)));
    window.dispatchEvent(
      new CustomEvent(GUIDE_POSITION_EVENT, { detail: { tripId, position } }),
    );
    if (!arrival) return;
    ...
    session.guide = {
      tripId,
      tripTitle: trip.title,
      stopId: arrival.stop.id,
      stopName: arrival.stop.name,
      day: arrival.stop.day || 1,
      latitude: arrival.stop.latitude!,
      longitude: arrival.stop.longitude!,
      status: "loading",
    };
    session.messages = [
      {
        id: createId("message"),
        role: "assistant",
        content: `已到达${arrival.stop.name}，正在准备景点讲解…`,
        createdAt: Date.now(),
      },
    ];
    ...
    window.dispatchEvent(
      new CustomEvent(GUIDE_OPEN_EVENT, { detail: session.id }),
    );
  };
  ...
  const sync = () => {
    ...
    if (!active) return;
    stopWatch = startNativeLocationWatch(
      (position) => onPosition(active.id, position),
      onError,
    );
