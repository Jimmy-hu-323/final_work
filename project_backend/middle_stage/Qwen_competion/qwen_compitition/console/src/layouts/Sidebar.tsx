...
export default function Sidebar({ selectedKey }: SidebarProps) {
  ...
  const simpleFlatNav = useMemo(() => {
    ...
    const stickyTravelPlanner: FlatMenuEntry = {
      key: "core.travel-planner",
      icon: <SparkBrowseLine size={16} />,
      path: "/travel-planner",
      label: t("nav.travelPlanner"),
    };
    const stickyTravelAlbum: FlatMenuEntry = {
      key: "core.travel-album",
      icon: <SparkPictureLine size={16} />,
      path: "/travel-album",
      label: t("nav.travelAlbum"),
    };
    return [
      stickyChat,
      stickyTravelPlanner,
  ...
  }, [agentMenu, settingsMenu, routes, sidebarMode, chatPath, t]);
  ...
  const isTravelPlannerActive = selectedKey === "core.travel-planner";
  const isTravelAlbumActive = selectedKey === "core.travel-album";
        ...
        <nav className={styles.collapsedNav}>
          {collapsedNavItems.map((item) => {
            const isActive =
              item.key === "core.chat"
                ? isChatActive
                : item.key === "core.travel-planner"
                  ? isTravelPlannerActive
                  : item.key === "core.travel-album"
                    ? isTravelAlbumActive
            ...
            <div className={styles.simpleNavItems}>
              {simpleFlatNav.map((entry) => {
                ...
                const isActive =
                  entry.key === "core.chat"
                    ? isChatActive
                    : entry.key === "core.travel-planner"
                      ? isTravelPlannerActive
                      : entry.key === "core.travel-album"
                        ? isTravelAlbumActive
        ...
        <>
          ...
          <div className={styles.agentScopedSection}>
            <div className={styles.agentSelectorContainer}>
              ...
              >
                <SparkBrowseLine size={16} />
                <span>{t("nav.travelPlanner")}</span>
              </button>
              <button
                className={`${styles.stickyChatButton}${
                  isTravelAlbumActive
                ...
                onClick={() => navigate("/travel-album")}
              >
                <SparkPictureLine size={16} />
                <span>{t("nav.travelAlbum")}</span>
              </button>
            </div>
