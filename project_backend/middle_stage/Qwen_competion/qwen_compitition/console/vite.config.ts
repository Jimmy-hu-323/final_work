...
// Leaflet is excluded: its stylesheet carries the positioning rules the map
// panes require, so stubbing it would leave markers/tiles laid out in normal
// flow (the map fails to render). Its own CSS is safe for Vite to bundle.
const cssStubPlugin = {
  name: "css-stub",
...
export default defineConfig(({ mode }) => {
  ...
  const mobile = mode === "mobile" || env.VITE_MOBILE === "true";

  return {
    base: mobile ? "./" : undefined,
    define: {
