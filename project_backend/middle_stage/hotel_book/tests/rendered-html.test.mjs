...
test("ships the hotel booking product instead of the starter", async () => {
  const [page, app, layout, css, hosting, openapi, inventory, staticEntry] = await Promise.all([
    ...
    readFile(new URL("app/api/v1/openapi/route.ts", projectRoot), "utf8"),
    readFile(new URL("app/api/v1/inventory/route.ts", projectRoot), "utf8"),
  ...
  assert.match(openapi, /request_payment_authorization/);
  assert.doesNotMatch(page, /codex-preview|SkeletonPreview/);
  assert.doesNotMatch(app, /Your site is taking shape/);
  assert.doesNotMatch(app, /hotel_harbour/);
  assert.doesNotMatch(inventory, /hotel_harbour/);
  assert.doesNotMatch(inventory, /active\s*=\s*1/);
