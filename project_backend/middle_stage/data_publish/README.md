...
给 LensGo × QwenPaw 澳门旅行项目做的**模拟数据发布工具**。除了手动发布各城市
区 / 街道 / 景点的人数，v1.2 还可发布澳门巴士的模拟位置、下一站、到站时间、
拥挤度和延误，并支持在网页中自动推进车辆。
...
| **巴士报站** | 手动发布或自动模拟巴士位置、下一站、到站时间和车内拥挤度 |
...
| GET | `/api/bus/routes` | 模拟路线及有序站点 |
| GET | `/api/bus/vehicles?route_id=` | 当前模拟车辆、位置、下一站和预计到站时间 |
| POST | `/api/bus/vehicles` | 发布或更新一辆模拟巴士 |
| GET | `/api/bus/stops/{id}/arrivals` | 指定站点的模拟到站预测 |
...
内置 4 条澳门演示路线。发布器可以选择路线、当前站、驶向下一站的进度、车辆状态、
车内拥挤度、延误和速度，既可以发布一次，也可以按指定间隔自动推进。

所有巴士接口都返回 `source: "mock"` 和免责声明。路线与站点名称用于演示，位置与
到站时间不是交通事务局实时数据，不可作为实际乘车依据。以后取得正式授权接口后，
可以保留这些 API 返回结构，只替换数据采集层。
